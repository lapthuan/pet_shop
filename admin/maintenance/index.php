        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				<colgroup>
					...
					<col width="10%">
					<col width="15%">
				</colgroup><?php if($_settings->chk_flashdata('success')): ?>
<script>
	alert_toast("<?php echo $_settings->flashdata('success') ?>",'success')
</script>
<?php endif;?>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Bạn có chắc chắn xóa danh mục này vĩnh viễn?","delete_category",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
		...
		$.ajax({
			...
			success:function(resp){
				...
				<tbody>
					<?php 
					$i = 1;
					$qry = $conn->query("SELECT * from `packages` order by unix_timestamp(date_created) desc ");
					while($row = $qry->fetch_assoc()):
						$row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));
					?>
